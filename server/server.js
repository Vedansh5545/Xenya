...
/* ------------------------------------------------------------------ */
/* Goals/Capacity file-backed API (shared by To-Do/Brief/Goals tabs)  */
/* ------------------------------------------------------------------ */
...
function ensureDataFile() {
  ...
  if (!fs.existsSync(STATE_PATH)) {
    const seed = {
      goals: {
        long: [{ id: 'seed-long', title: 'Get an AI/ML job', weight: 80, targetDate: '' }],
        short: [{ id: 'seed-short', title: 'Daily DSA warm-up', weight: 60, targetDate: '' }],
      },
...
app.post('/api/state', express.json(), (req, res) => {
  try {
    const next = { ...(req.body || {}), updatedAt: new Date().toISOString() };
...
function parseJDText(jd_text=''){
  ...
  const visa_status = /no\s+visa\s*sponsorship|unable\s+to\sponsor|us\s*citizen|citizenship|green\s*card/i.test(visaQuote)
    ? 'unfriendly'
...
Endpoints:
  ...
  /api/search, /api/summary?url=&model=, /api/rss, /api/research (GET/POST)
